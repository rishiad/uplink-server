...
const SERVER_TSCONFIG = path.join(REPO_ROOT, 'src', 'tsconfig.server.json');
...
['reh', 'reh-web'].forEach(type => {
	...
	BUILD_TARGETS.forEach(buildTarget => {
		...
		const label = type === 'reh' ? 'server' : type;

		['', 'min'].forEach(minified => {
			const sourceFolderName = `out-vscode-${type}${dashed(minified)}`;
			const destinationFolderName = `vscode-${label}${dashed(platform)}${dashed(arch)}`;
			const nameBase = `vscode-${label}${dashed(platform)}${dashed(arch)}`;
			const legacyBase = `vscode-${type}${dashed(platform)}${dashed(arch)}`;

			const serverTaskCI = task.define(`${nameBase}${dashed(minified)}-ci`, task.series(
			...
			const serverTask = task.define(`${nameBase}${dashed(minified)}`, task.series(
				() => { process.env['VSCODE_SERVER_TSCONFIG'] = SERVER_TSCONFIG; },
			...
			if (!minified) {
				const lowMemTask = task.define(`${nameBase}-lowmem`, task.series(
					() => {
						process.env['VSCODE_NO_SOURCEMAP'] = '1';
						process.env['VSCODE_SERVER_TSCONFIG'] = SERVER_TSCONFIG;
					},
