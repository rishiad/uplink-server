...
async function loadWebpackConfig(webpackConfigPath) {
    const ext = path_1.default.extname(webpackConfigPath);
    if (ext === '.cjs') {
        const mod = require(webpackConfigPath);
        return (mod === null || mod === void 0 ? void 0 : mod.default) ?? mod;
    }
    if (ext === '.mjs') {
        const moduleUrl = (0, url_1.pathToFileURL)(webpackConfigPath).href;
        const mod = await import(moduleUrl);
        return (mod === null || mod === void 0 ? void 0 : mod.default) ?? mod;
    }
    const source = fs_1.default.readFileSync(webpackConfigPath, 'utf8');
    const looksEsm = /\bimport\s|export\s+default/.test(source);
    if (!looksEsm) {
        const mod = require(webpackConfigPath);
        return (mod === null || mod === void 0 ? void 0 : mod.default) ?? mod;
    }
    const tempPath = `${webpackConfigPath}.${process.pid}.${crypto_1.default.randomBytes(4).toString('hex')}.mjs`;
    await fs_1.default.promises.writeFile(tempPath, source, 'utf8');
    try {
        const moduleUrl = (0, url_1.pathToFileURL)(tempPath).href;
        const mod = await import(moduleUrl);
        return (mod === null || mod === void 0 ? void 0 : mod.default) ?? mod;
    }
    finally {
        await fs_1.default.promises.unlink(tempPath).catch(() => undefined);
    }
}
function collectWebpackExternals(config) {
    if (!config) {
        return [];
    }
    ...
    for (const cfg of configs) {
        if (cfg && typeof cfg.externals === 'object' && !Array.isArray(cfg.externals)) {
            externals.push(...Object.keys(cfg.externals));
        }
    }
    return externals;
}
function fromLocalWebpack(extensionPath, webpackConfigFileName, disableMangle) {
    ...
    const webpackRootConfigPath = path_1.default.join(extensionPath, webpackConfigFileName);
    let packagedDependencies = [];
    ...
    vsce.listFiles({ cwd: extensionPath, packageManager: vsce.PackageManager.None }).then(async (fileNames) => {
        packagedDependencies = [];
        if (packageJsonConfig.dependencies) {
            try {
                const webpackRootConfig = await loadWebpackConfig(webpackRootConfigPath);
                for (const key of collectWebpackExternals(webpackRootConfig)) {
        ...
        let webpackConfigEntries;
        try {
            webpackConfigEntries = await Promise.all(webpackConfigLocations.map(async (webpackConfigPath) => ({
                path: webpackConfigPath,
                config: await loadWebpackConfig(webpackConfigPath)
            })));
        }
        catch (err) {
            result.emit('error', err);
        ...
        const webpackStreams = webpackConfigEntries.flatMap(({ path: webpackConfigPath, config: exportedConfig }) => {
