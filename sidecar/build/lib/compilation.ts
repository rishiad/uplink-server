...
export function createCompile(src: string, { build, emitError, transpileOnly, preserveEnglish }: ICompileTaskOptions) {
	...
	const projectPath = process.env['VSCODE_SERVER_TSCONFIG']
		? path.resolve(process.env['VSCODE_SERVER_TSCONFIG'])
...
export function transpileTask(src: string, out: string, esbuild?: boolean): task.StreamTask {
	...
	const task = () => {
		...
		const nonts = gulp.src(`${src}/**`, { base: `${src}` }).pipe(nontsFilter);
		const tsSources = transpile.tsProjectSrc ? transpile.tsProjectSrc() : gulp.src(`${src}/**/*.ts`, { base: `${src}` });
		const srcPipe = es.merge(nonts, tsSources);
...
export function compileTask(src: string, out: string, build: boolean, options: { disableMangle?: boolean; preserveEnglish?: boolean } = {}): task.StreamTask {
	...
	const task = () => {
		...
		const nontsFilter = util.filter(data => !/\.ts$/.test(data.path));
		const nonts = gulp.src(`${src}/**`, { base: `${src}` }).pipe(nontsFilter);
		const tsSources = compile.tsProjectSrc ? compile.tsProjectSrc() : gulp.src(`${src}/**/*.ts`, { base: `${src}` });
		const srcPipe = es.merge(nonts, tsSources);
