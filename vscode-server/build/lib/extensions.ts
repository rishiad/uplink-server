...
async function loadWebpackConfig(webpackConfigPath: string): Promise<any> {
	const ext = path.extname(webpackConfigPath);
	if (ext === '.cjs') {
	...
	if (ext === '.mjs') {
		const moduleUrl = pathToFileURL(webpackConfigPath).href;
	...
	const tempPath = `${webpackConfigPath}.${process.pid}.${crypto.randomBytes(4).toString('hex')}.mjs`;
...
function collectWebpackExternals(config: any): string[] {
	if (!config) {
		return [];
	}
...
function fromLocalWebpack(extensionPath: string, webpackConfigFileName: string, disableMangle: boolean): Stream {
	...
	const webpackRootConfigPath = path.join(extensionPath, webpackConfigFileName);
	...
	vsce.listFiles({ cwd: extensionPath, packageManager: vsce.PackageManager.None }).then(async fileNames => {
		...
		let webpackConfigEntries: Array<{ path: string; config: any }>;
		try {
