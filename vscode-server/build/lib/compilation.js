...
function createCompile(src, { build, emitError, transpileOnly, preserveEnglish }) {
	...
	const projectPath = process.env['VSCODE_SERVER_TSCONFIG']
		? path_1.default.resolve(process.env['VSCODE_SERVER_TSCONFIG'])
		: path_1.default.join(__dirname, '../../', src, 'tsconfig.json');
...
function transpileTask(src, out, esbuild) {
	const task = () => {
		...
		const nontsFilter = util.filter(data => !/\.ts$/.test(data.path));
		const nonts = gulp_1.default.src(`${src}/**`, { base: `${src}` }).pipe(nontsFilter);
		const tsSources = transpile.tsProjectSrc ? transpile.tsProjectSrc() : gulp_1.default.src(`${src}/**/*.ts`, { base: `${src}` });
		const srcPipe = event_stream_1.default.merge(nonts, tsSources);
		return srcPipe
			.pipe(transpile())
...
function compileTask(src, out, build, options = {}) {
	const task = () => {
		...
		const nontsFilter = util.filter(data => !/\.ts$/.test(data.path));
		const nonts = gulp_1.default.src(`${src}/**`, { base: `${src}` }).pipe(nontsFilter);
		const tsSources = compile.tsProjectSrc ? compile.tsProjectSrc() : gulp_1.default.src(`${src}/**/*.ts`, { base: `${src}` });
		const srcPipe = event_stream_1.default.merge(nonts, tsSources);
